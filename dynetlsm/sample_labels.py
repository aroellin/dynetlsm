...
from .gaussian_likelihood_fast import spherical_normal_log_pdf
...
def sample_labels_gibbs(X, mu, sigma, lmbda, w0, w, random_state=None):
        ...
        for i in range(n_nodes):
            ...
            if t == 0:
                for k in range(n_components):
                    probas[k] = (np.log(w0[k] + 1e-5) +
                                 spherical_normal_log_pdf(X[t, i],
                                                          mu[k],
                                                          sigma[k]))
            else:
                for k in range(n_components):
                    probas[k] = (np.log(w[z[t-1, i], k] + 1e-5) +
...
def sample_labels_block_lpcm(
    X, mu, sigma, lmbda, init_weights, trans_weights, random_state=None):
    random_state = check_random_state(random_state)
    ...
    for i in range(n_nodes):
        ...
        for t in range(n_time_steps - 1, 0, -1):
            ...
            bwds_msg[t-1] = np.dot(
                trans_weights, partial_marg[t].reshape(-1, 1)).ravel()
        ...
        for t in range(n_time_steps):
            if t == 0:
                probas = init_weights * partial_marg[0]
            else:
                probas = trans_weights[z[t-1, i]] * partial_marg[t]
